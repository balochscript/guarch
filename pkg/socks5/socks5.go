...
func Handshake(conn net.Conn) (string, error) {
	...
	if int(nMethods) > maxNMethods {
		return "", fmt.Errorf("socks5: too many methods: %d", nMethods)
	}
	...
	case 0x03:
		...
		if int(lenBuf[0]) > 253 {
			return "", fmt.Errorf("socks5: domain too long: %d", lenBuf[0])
		}
